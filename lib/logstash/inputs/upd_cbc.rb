...
class LogStash::Inputs::Udp < LogStash::Inputs::Base
  ...
  config :key, :validate => :string, :default => '', :required => true
  config :iv, :validate => :string, :default => '', :required => true
  config :size_cipher, :validate => :number, :default => 128
  ...
  def udp_listener(output_queue)
    ...
    cipher = OpenSSL::Cipher::AES.new(@size_cipher, :CBC)
    cipher.decrypt
    cipher.key = @key
    cipher.iv = @iv
    ...
    while !stop?
      ...
      @queue_size.times do
        begin
          ...
          data = cipher.update(payload) + cipher.final
          push_data(data, client)
